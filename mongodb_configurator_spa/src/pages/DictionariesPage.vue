    ...
    <v-row>
      <v-col cols="12">
        <div class="d-flex justify-space-between align-center mb-6">
          <h1 class="text-h4">Dictionaries</h1>
        </div>
        ...
        <v-dialog v-model="showUpload" max-width="600">
          <v-card>
            <v-card-title>Upload Dictionaries</v-card-title>
            <v-card-text>
              <FileUpload 
                file-type="dictionaries"
...
const {
  ...
  deleteFile,
  toggleFileLock
} = useFiles('dictionaries')

// Use upload composable
const { downloadFile } = useFileUpload('dictionaries')
...
const createNew = () => {
  // TODO: Implement new dictionary creation
...
const processSelected = async () => {
  // Dictionaries don't have processing, so this is a no-op
  console.log('Processing not available for dictionaries')
}

const deleteSelected = async () => {
  // For now, delete all files (with confirmation)
  if (confirm('Are you sure you want to delete all dictionaries?')) {
    for (const file of files.value) {
      try {
