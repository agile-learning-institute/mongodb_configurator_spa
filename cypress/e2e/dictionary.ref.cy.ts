...
describe('Dictionary Details Page', () => {
  ...
  beforeEach(() => {
    ...
    // Ensure there is at least one referenced (non-versioned) dictionary available
    referencedDictionaryName = `refDict-${Date.now()}`
    ...
    // Set type to Object on the test dictionary
  ...
  describe('Ref Property Editor', () => {
    ...
    it('persists ref changes when selecting a referenced dictionary', () => {
      cy.visit(`/dictionaries/${dictionaryFileName}`)
      ...
      cy.get(`[data-test="ref-dictionary-option-name-${referencedRefName}"]`).should('be.visible').click()
      cy.get('[data-test="ref-dictionary-picker-card"]').should('not.exist')
      cy.get('[data-test="ref-dictionary-chip"]').eq(1).should('contain', referencedRefName)
      cy.wait(250)
      cy.reload()
      cy.get('[data-test="ref-dictionary-chip"]').eq(1).should('be.visible').should('contain', referencedRefName)
    })
  })
})
