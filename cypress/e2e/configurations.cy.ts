describe('Configurations page flow', () => {
  ...
  describe('Step 5 Index Management', () => {
    it.only('can add, edit, and remove Step 5 indexes', () => {
      ...
      // Verify dialog shows default JSON structure - target the main textarea (second) within Vuetify component
      cy.get('[data-test="step5-index-json-textarea"] textarea').eq(1).invoke('val').should('contain', '"name": ""')
      cy.get('[data-test="step5-index-json-textarea"] textarea').eq(1).invoke('val').should('contain', '"key": {}')
      cy.get('[data-test="step5-index-json-textarea"] textarea').eq(1).invoke('val').should('contain', '"options": {}')
      ...
      cy.get('[data-test="step5-index-json-textarea"] textarea').eq(1).invoke('val').should('contain', '"name": "name1"')
      
      // Edit the JSON to change the name
      cy.get('[data-test="step5-index-json-textarea"] textarea').eq(1).clear().type('{"name": "edited_name", "key": {}, "options": {}}')
      cy.get('[data-test="save-index-btn"]').click()
