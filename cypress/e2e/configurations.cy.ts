describe('Configurations page flow', () => {
  ...
  describe('Step 5 Index Management', () => {
    it.only('can add, edit, and remove Step 5 indexes', () => {
      ...
      cy.get('[data-test="step5-index-json-textarea"] textarea').first().should('contain', '"name": ""')
      cy.get('[data-test="step5-index-json-textarea"] textarea').first().should('contain', '"key": {}')
      cy.get('[data-test="step5-index-json-textarea"] textarea').first().should('contain', '"options": {}')
      
      // Test validation - try to save without a name
      cy.get('[data-test="step5-index-json-textarea"] textarea').first().clear().type('{"key": {}, "options": {}}')
      cy.get('[data-test="save-index-btn"]').should('be.disabled')
      
      // Test validation - try to save with invalid JSON
      cy.get('[data-test="step5-index-json-textarea"] textarea').first().clear().type('invalid json')
      cy.get('[data-test="save-index-btn"]').should('be.disabled')
      
      // Edit the JSON to set the name
      cy.get('[data-test="step5-index-json-textarea"] textarea').first().clear().type('{"name": "name1", "key": {}, "options": {}}')
      ...
      cy.get('[data-test="step5-index-json-textarea"] textarea').first().should('contain', '"name": "name1"')
