describe('Configurations page flow', () => {
  ...
  describe('Step 5 Index Management', () => {
    it.only('can add, edit, and remove Step 5 indexes', () => {
      ...
      // Verify dialog shows default JSON structure - target the actual textarea within Vuetify component
      cy.get('[data-test="step5-index-json-textarea"] textarea').should('contain', '"name": ""')
      cy.get('[data-test="step5-index-json-textarea"] textarea').should('contain', '"key": {}')
      cy.get('[data-test="step5-index-json-textarea"] textarea').should('contain', '"options": {}')
      
      // Test validation - try to save without a name
      cy.get('[data-test="step5-index-json-textarea"] textarea').clear().type('{"key": {}, "options": {}}')
      cy.get('[data-test="save-index-btn"]').should('be.disabled')
      
      // Test validation - try to save with invalid JSON
      cy.get('[data-test="step5-index-json-textarea"] textarea').clear().type('invalid json')
      cy.get('[data-test="save-index-btn"]').should('be.disabled')
      
      // Edit the JSON to set the name
      cy.get('[data-test="step5-index-json-textarea"] textarea').clear().type('{"name": "name1", "key": {}, "options": {}}')
      ...
      cy.get('[data-test="step5-index-json-textarea"] textarea').should('contain', '"name": "name1"')
      
      // Edit the JSON to change the name
      cy.get('[data-test="step5-index-json-textarea"] textarea').clear().type('{"name": "edited_name", "key": {}, "options": {}}')
      cy.get('[data-test="save-index-btn"]').click()
